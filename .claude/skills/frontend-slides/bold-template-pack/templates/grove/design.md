...
- **Bullet glyphs are em-dashes (`U+2014`) rendered in JetBrains Mono `{colors.accent}` coral.** Never bullets (`•`), never hyphens (`-`), never asterisks. The em-dash is the system's bullet language.
...
- Use the em-dash glyph (`U+2014`) in coral as the universal bullet. Never use round bullets or hyphens.
