...
components:
  ...
  bullet-em-dash:
    ...
    fontWeight: 700
    paddingLeft: 14px
    description: "List bullet using a red em-dash (U+2014) glyph in place of a disc. The dash sits at position absolute left and the body text indents to clear it."
  bullet-bullet:
    glyph: "•"
    color: "{colors.red}"
    paddingLeft: 12px
...
- **Every list bullet uses a `{colors.red}` em-dash (U+2014) or round-bullet (•) glyph** at position absolute left with a 12-14px padding-left on the body. Default disc bullets do not exist; the red marker is the system's list signal.
