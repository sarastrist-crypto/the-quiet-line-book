...
function App() {
  ...
  const handleStripeCheckout = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:8000'}/api/create-checkout-session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            ...
            <div className="hero-ctas">
              ...
              <div className="secondary-ctas">
                ...
                <div className="retailer-links">
                  <button onClick={handleStripeCheckout} className="btn-link" disabled={loading}>Direct (Signed Edition)</button>
                  <a href="https://amazon.com" target="_blank" rel="noopener noreferrer">Amazon</a>
                  <a href="https://bookshop.org" target="_blank" rel="noopener noreferrer">Bookshop</a>
                </div>
              </div>
            </div>
          </div>
